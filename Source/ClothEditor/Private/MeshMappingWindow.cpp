    ...
    ChildSlot
        [SNew(SVerticalBox)
         ...
         + SVerticalBox::Slot().AutoHeight().Padding(5)
               [SNew(SHorizontalBox) + SHorizontalBox::Slot().AutoWidth().Padding(0, 5, 10, 0).VAlign(VAlign_Center)[SNew(STextBlock).Text(LOCTEXT("LowPoly", "Low Poly Mesh:"))] + SHorizontalBox::Slot().FillWidth(1.0f)[SNew(SObjectPropertyEntryBox).AllowedClass(USkeletalMesh::StaticClass()) // 只允许选骨骼模型
         ...
         // --- High Poly Mesh 选择器 ---
         + SVerticalBox::Slot().AutoHeight().Padding(5)
               [SNew(SHorizontalBox) + SHorizontalBox::Slot().AutoWidth().Padding(0, 5, 10, 0).VAlign(VAlign_Center)[SNew(STextBlock).Text(LOCTEXT("HighPoly", "High Poly Mesh:"))] + SHorizontalBox::Slot().FillWidth(1.0f)[SNew(SObjectPropertyEntryBox).AllowedClass(USkeletalMesh::StaticClass()).ObjectPath(this, &SMeshMappingWindow::GetHighMeshPath).OnObjectChanged(this, &SMeshMappingWindow::OnHighMeshChanged).AllowClear(true).DisplayUseSelected(true)]]

         // --- 策略选择 ---
         + SVerticalBox::Slot().AutoHeight().Padding(5)
               [SNew(SHorizontalBox) + SHorizontalBox::Slot().AutoWidth().Padding(0, 5, 10, 0).VAlign(VAlign_Center)[SNew(STextBlock).Text(LOCTEXT("Strategy", "Mapping Algorithm:"))] + SHorizontalBox::Slot().FillWidth(1.0f)[SAssignNew(StrategyComboBox, SComboBox<TSharedPtr<IMeshMappingStrategy>>).OptionsSource(&AvailableStrategies).OnGenerateWidget(this, &SMeshMappingWindow::GenerateStrategyComboItem).OnSelectionChanged(this, &SMeshMappingWindow::OnStrategySelectionChanged)[SNew(STextBlock).Text(this, &SMeshMappingWindow::GetCurrentStrategyLabel)]]]
         ...
         // --- 输出路径设置 ---
         + SVerticalBox::Slot().AutoHeight().Padding(5)
               [SNew(SHorizontalBox) + SHorizontalBox::Slot().AutoWidth().Padding(0, 5, 10, 0).VAlign(VAlign_Center)[SNew(STextBlock).Text(LOCTEXT("OutFolder", "Output Folder:"))] + SHorizontalBox::Slot().FillWidth(1.0f)[SAssignNew(OutputPathBox, SEditableTextBox).Text(FText::FromString(SaveFolderPath)).OnTextChanged(this, &SMeshMappingWindow::OnOutputPathChanged)]] +
         SVerticalBox::Slot().AutoHeight().Padding(5)
             [SNew(SHorizontalBox) + SHorizontalBox::Slot().AutoWidth().Padding(0, 5, 10, 0).VAlign(VAlign_Center)[SNew(STextBlock).Text(LOCTEXT("OutName", "Output Name:"))] + SHorizontalBox::Slot().FillWidth(1.0f)[SAssignNew(OutputNameBox, SEditableTextBox).Text(FText::FromString(SaveFileName)).OnTextChanged(this, &SMeshMappingWindow::OnOutputNameChanged)]]

         // --- Bake 按钮 ---
         + SVerticalBox::Slot().AutoHeight().Padding(10, 20).HAlign(HAlign_Right)
               [SNew(SButton)
